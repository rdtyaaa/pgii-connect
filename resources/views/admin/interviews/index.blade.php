<x-admin-layout>
    ...
    <div class="mb-4">
        <form method="GET" action="{{ route('interviews.index') }}">
            <input type="text" name="search" value="{{ request()->get('search') }}" placeholder="Search by student name"
                class="w-full rounded-lg border border-gray-300 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 -:border-gray-500 -:bg-gray-600 -:text-white">
        </form>
    </div>

    <!-- Interviews Table -->
    <div class="relative overflow-x-auto border shadow sm:rounded-lg">
        <table class="min-w-full text-left text-sm text-gray-500 -:text-gray-400">
                    ...
                    <tr
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('interviews.detail', $interview->id) }}"
                                class="text-blue-600 hover:underline -:text-blue-500">Detail</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div class="mt-4 flex flex-col items-center">
        <span class="text-sm text-gray-700 -:text-gray-400">
            Showing <span class="font-semibold text-gray-900 -:text-white">{{ $interviews->firstItem() }}</span> to
            <span class="font-semibold text-gray-900 -:text-white">{{ $interviews->lastItem() }}</span> of <span
                class="font-semibold text-gray-900 -:text-white">{{ $interviews->total() }}</span> Entries
        </span>
